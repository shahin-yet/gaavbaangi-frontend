...
window.addEventListener('DOMContentLoaded', function () {
  ...
  function renderRefugeList(refuges = [], query = '') {
    ...
    // Preserve membership buttons across re-renders (including admin/user mode switches)
    // so they are not destroyed/recreated.
    const prevMembershipButtonsById = new Map();
    try {
      document.querySelectorAll('.refuge-list-item').forEach((el) => {
        const rid = el.getAttribute('data-refuge-id');
        const btn = el.querySelector('.refuge-membership-btn');
        if (rid && btn) prevMembershipButtonsById.set(String(rid), btn);
      });
    } catch (e) {}

    // If the membership button was focused, attempt to restore focus after re-render.
    let restoreFocusRid = null;
    try {
      const active = document.activeElement;
      if (active && active.classList && active.classList.contains('refuge-membership-btn')) {
        const parentItem = active.closest('.refuge-list-item');
        const rid = parentItem && parentItem.getAttribute('data-refuge-id');
        if (rid) restoreFocusRid = String(rid);
      }
    } catch (e) {}
    ...
    sorted.forEach((refuge) => {
      ...
      item.innerHTML = `
        ...
        <div class="refuge-list-meta">
          <span class="refuge-membership-slot" aria-hidden="true"></span>
          ${defaultControlHtml}
        </div>
      `;
      ...
      // Reuse existing membership button node (per refuge) so mode switches don't replace it.
      const membershipSlot = item.querySelector('.refuge-membership-slot');
      const ridForMembership = item.getAttribute('data-refuge-id');
      let membershipBtn = ridForMembership ? prevMembershipButtonsById.get(String(ridForMembership)) : null;
      if (!membershipBtn) {
        membershipBtn = document.createElement('button');
        membershipBtn.type = 'button';
        membershipBtn.className = 'refuge-membership-btn';
      ...
      if (membershipSlot && membershipBtn) {
        membershipSlot.replaceWith(membershipBtn);
      }
    ...
    if (restoreFocusRid) {
      try {
        const btn = prevMembershipButtonsById.get(String(restoreFocusRid));
